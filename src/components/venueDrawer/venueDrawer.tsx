...
export const VenueDrawer = memo(({ open, venue, onClose }: VenueSheetProps) => {
    const lastVenue = useRef<Venue | null>(null);
    if (venue) lastVenue.current = venue;
    const displayVenue = venue ?? lastVenue.current;
    ...
    return <Drawer open={open} onClose={onClose} direction={positionSetting}>
        <DrawerContent className="max-w-150">
            <DrawerHeader className="p-0">
                ...
                <img className={bannerStyle({ side: positionSetting })} src={displayVenue.bannerUri ?? defaultBanner} alt={displayVenue.name} />
                <VenueToolbar venue={displayVenue} />
                <DrawerTitle className="text-2xl mx-8 mt-4 mb-0">{displayVenue.name}</DrawerTitle>
                <DrawerDescription className="text-muted-foreground text-md mx-8 mb-2">
                    <LocationText location={displayVenue.location} />
                </DrawerDescription>
            </DrawerHeader>
            <div className="mx-8 no-scrollbar overflow-y-auto">
                <VenueDescription description={displayVenue.description} />
                <VenueTags tags={displayVenue.tags} />
                <hr className="border-t border-dotted border-muted mt-8 "/>
                <VenueSchedule venue={displayVenue} className="mt-8"/>
                ...
                <VenueNsfwText className="mt-8" hasCourts={displayVenue.tags.indexOf('Courtesans') >= 0} openlyNsfw={!displayVenue.sfw} />
            </div>
            <DrawerFooter>
                <div className="flex justify-content-end gap-4 m-4">
                    <VenueWebsiteButton website={displayVenue.website} />
                    <VenueDiscordButton discordLink={displayVenue.discord} />
                </div>
            </DrawerFooter>
        </DrawerContent>
    </Drawer>
